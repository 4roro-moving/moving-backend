...
export const predictPriceSchema = z.object({
  ...
  distanceKm: z
    .number({ error: "이동 거리는 숫자여야 합니다." })
    .positive("이동 거리는 0보다 커야 합니다.")
    .max(1000, "이동 거리는 1,000km 이하여야 합니다."),
  houseSize: z
    .number({ error: "평수는 숫자여야 합니다." })
    .int("평수는 정수여야 합니다.")
    .min(1, "평수는 1 이상이어야 합니다.")
    .max(300, "평수는 300 이하여야 합니다."),
  loadAmount: z.enum(LOAD_AMOUNTS, { error: "올바른 짐량을 선택해 주세요." }),
  fromFloor: z
    .number({ error: "출발 층수는 숫자여야 합니다." })
    .int("출발 층수는 정수여야 합니다.")
    .min(1, "출발 층수는 1 이상이어야 합니다.")
    .max(100, "출발 층수는 100 이하여야 합니다."),
  fromElevator: z.boolean(),
  toFloor: z
    .number({ error: "도착 층수는 숫자여야 합니다." })
    .int("도착 층수는 정수여야 합니다.")
    .min(1, "도착 층수는 1 이상이어야 합니다.")
    .max(100, "도착 층수는 100 이하여야 합니다."),
