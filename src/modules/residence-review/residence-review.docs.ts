...
const RESIDENCE_REVIEW_DOCS = {
  ...
  PUBLIC_LIST_DESCRIPTION: [
    ...
    "- `sort=createdAt`(기본)은 최신순, `sort=rating`은 별점 높은 순입니다. 별점이 같으면 최신순입니다.",
    "- `keyword`·`regionId`·`sort`가 바뀌면 `cursor` 없이 처음부터 조회해야 합니다.",
  ...
  PUBLIC_DETAIL_DESCRIPTION: "숨김 처리된 후기는 작성자를 포함해 조회할 수 없습니다.",
