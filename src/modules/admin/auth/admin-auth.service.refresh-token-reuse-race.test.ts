...
const ADMIN_ID = "admin-reuse-race-1";
...
function createAdminSessionUser(): AdminSessionUser {
  return {
    id: ADMIN_ID,
    email: "admin-reuse-race@example.com",
    name: "관리자",
    role: UserRole.ADMIN,
    isActive: true,
...
describe("adminAuthService refresh token reuse detection race condition", () => {
  ...
  it("does not leave an active refresh token when R1 reuse races with R2 rotation in the same admin family", async () => {
    const refreshTokenR1 = createRefreshToken({ userId: ADMIN_ID, role: UserRole.ADMIN });
    const refreshTokenR2 = createRefreshToken({ userId: ADMIN_ID, role: UserRole.ADMIN });
    ...
    const r1ReusePromise = adminAuthService.refresh(refreshTokenR1);
    const r2RotationPromise = adminAuthService.refresh(refreshTokenR2);
    ...
    const activeFamilyTokens = getActiveFamilyTokens(
      store,
      FAMILY_A,
    ...
    assert.equal(
      ...
      0,
      `race left ${activeFamilyTokens.length} active ADMIN refresh token(s) in family ${FAMILY_A}`,
    );
