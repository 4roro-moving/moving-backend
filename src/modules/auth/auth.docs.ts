  ...
  endpoints: {
    ...
    "POST /login": {
      ...
      description: [
        ...
        "- 정지 계정이 비밀번호 본인 인증에 성공하면 403 `ACCOUNT_SUSPENDED`와 함께 정지 사유 및 `appealAvailable: true`를 반환하고, 30분 만료의 이의 제기 전용 토큰을 문의 API 전용 HttpOnly Cookie로 설정합니다. 일반 Access Token과 Refresh Token은 발급하지 않습니다.",
    ...
    "POST /oauth/google": {
      ...
      description: [
        ...
        "- 로그인 성공 시 사용자 정보와 Access Token을 반환합니다.",
        "- Refresh Token은 HttpOnly Cookie로 저장됩니다.",
        "- 정지 계정은 403 `ACCOUNT_SUSPENDED`와 함께 정지 사유 및 `appealAvailable: true`를 반환하고, 30분 만료의 이의 제기 전용 토큰을 문의 API 전용 HttpOnly Cookie로 설정합니다. 일반 Access Token과 Refresh Token은 발급하지 않습니다.",
      ].join("\n"),
      responses: {
        200: "Google 로그인 성공",
