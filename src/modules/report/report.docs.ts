  ...
  endpoints: {
    "POST /images/upload-url": {
      ...
      description: [
        ...
        "- 이미지 Key는 `temp/reports/{userId}/{uuid}.{extension}` 형식으로 생성됩니다.",
        "- 발급된 URL의 유효시간은 180초입니다.",
        "- S3 PUT 요청에는 발급 시 사용한 `Content-Type`과 `If-None-Match: *` 헤더를 함께 전송해야 합니다.",
    ...
    "POST /": {
      ...
      description: [
        ...
        "- 신고 생성 시 `temp/reports/` 이미지 Key의 소유권, MIME, 파일 존재 여부, 파일 크기를 다시 검증합니다.",
        "- 검증된 이미지는 `reports/{userId}/...` 최종 경로로 승격한 뒤 최종 Key만 DB에 저장합니다.",
        "- 신고 생성 성공 후 temp 객체는 즉시 정리하며, 저장 요청 없이 남은 temp 객체는 S3 Lifecycle 정책으로 정리합니다.",
